...
def build_weekly():
    ...
    for isin, prices_dict in history.items():
        ...
        # --- Ny Trend Logik ---
        if is_active:
            # For dine egne fonde: Vis ALT (Både op og ned)
            if past_state == "DOWN" and curr_state == "UP":
                portfolio_alerts.append({"name": fund_name, "msg": "🚀 Skiftet til BULL (Køb/Hold)", "type": "BULL"})
            elif past_state == "UP" and curr_state == "DOWN":
                portfolio_alerts.append({"name": fund_name, "msg": "⚠️ Skiftet til BEAR (Overvej salg)", "type": "BEAR"})
        else:
            # For markedet: Vis KUN nye BULL-muligheder
            if past_state == "DOWN" and curr_state == "UP":
        ...
        rows.append({
            "isin": isin, "name": fund_name, "is_active": is_active,
            "week_change_pct": week_change, "trend_state": curr_state,
    ...
    output = Template(template_html).render(
        week_label=today.strftime("%V"), week_end_date=today.strftime("%d-%m-%Y"),
        rows=table_rows,
        portfolio_alerts=portfolio_alerts,
        market_opportunities=market_opportunities[:8], # Begræns markedet til top 8
    ...
    print("Dashboard opdateret med fokuserede alarmer!")
